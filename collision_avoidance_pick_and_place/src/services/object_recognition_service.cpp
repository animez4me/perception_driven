...
const std::string OBJECT_RECOGNITION_SERVICE = "object_recognition";
...
public:
    ...
    target_detection_server = nh.advertiseService(OBJECT_RECOGNITION_SERVICE,&TargetRecognition::target_recognition_callback,this);
    ...
    //point_cloud_subscriber = nh.subscribe("/kinect2/sd/points",1,&TargetRecognition::point_cloud_callback,this);

    ork_subscriber = nh.subscribe("/recognized_object_array", 1, &TargetRecognition::objectCallback,this);
...
protected:
  ...
  void objectCallback(const object_recognition_msgs::RecognizedObjectArray objects_msg)
  {
    ...
    if (id >= 0) {
      ...
      ROS_INFO("OPose x is: %f", Object_pose.pose.position.x);
      ROS_INFO("OPose y is: %f", Object_pose.pose.position.y);
      ROS_INFO("OPose z is: %f", Object_pose.pose.position.z);
    }
    else {
      confident = 0;
...
//    br.sendTransform(transformStamped);
    ...
    //CHANGED FROM AR_FRAME_ID_ TO CLICKEDPT
    // looking up transforms
    if(get_transform(world_frame_id_,msg.header.frame_id,world_to_sensor_tf) &&
       //get_transform(world_frame_id_ ,ar_frame_id_,world_to_ar_tf))
       get_transform(world_frame_id_ ,"ORK",world_to_ar_tf))
    {
      ...
      ROS_INFO_STREAM("X " << box_pick_position.getX());
      ROS_INFO_STREAM("Y " << box_pick_position.getY());
      ROS_INFO_STREAM("Z " << box_pick_position.getZ());
    ...
    try
    {
      transform_listener_ptr->waitForTransform(target,source,ros::Time(0),ros::Duration(4.0f));
      transform_listener_ptr->lookupTransform(target,source,ros::Time(0),trg_to_src_stamped);
    ...
    {
      ROS_ERROR_STREAM("transform lookup1 for '"<<ar_frame_id_<<"' failed");
      return false;
    }
    catch(tf::ExtrapolationException &e)
    {
      ROS_ERROR_STREAM("transform lookup2 for '"<<ar_frame_id_<<"' failed");
      return false;
    }
    catch(tf::TransformException &e)
    {
      ROS_ERROR_STREAM("transform lookup3 for '"<<ar_frame_id_<<"' failed");
      return false;
    }
		return true;
	}
    ...
    ROS_INFO_STREAM("Centroid is: "<<centroid);
